...
| §11 | reconcile's two-level fold: per-task state/elapsed/RSS, unit verdict unchanged, RSS not smeared across tasks, no `tasks` key for an individual job, a pending `_[a-b]` range expands into its tasks (every index form, unknown form degrades), the unit verdict is the most advanced live state and an all-pending array reads PENDING while staying live for `submit`, unchanged observation not re-appended | ✅ | `test_tasks.py` |
